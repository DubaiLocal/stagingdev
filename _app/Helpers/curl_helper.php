...
function perform_http_request_blog_explore()
{
    // $url = base_url() . '/blog/wp-json/wp/v2/posts?_embed&categories=176';
    $url = getenv('BLOG_BASE_URL') . '/wp-json/wp/v2/posts?_embed&categories=176';
    $curl = curl_init();
    ...
    curl_close($curl);
    $decoded_json_blog = json_decode($result, true);

    return $decoded_json_blog;
}
function perform_http_request_blog_explore_error()
{
    // $url = base_url() . '/log/wp-json/wp/v2/posts?_embed&categories=176';
    $url = getenv('BLOG_BASE_URL') . '/log/wp-json/wp/v2/posts?_embed&categories=176';
    $curl = curl_init();
    curl_setopt($curl, CURLOPT_URL, $url);
    // curl_setopt($curl, CURLOPT_TIMEOUT, 1);
    // curl_setopt($curl, CURLOPT_NOSIGNAL, 1);
...
function perform_http_request_blog($cat_id)
{
    // $url = base_url() . '/blog/wp-json/wp/v2/posts?_embed&categories=176';
    $url = getenv('BLOG_BASE_URL') . '/wp-json/wp/v2/posts?_embed&categories=' . $cat_id;
    $curl = curl_init();
    curl_setopt($curl, CURLOPT_URL, $url);
    // curl_setopt($curl, CURLOPT_TIMEOUT, 1);
    // curl_setopt($curl, CURLOPT_NOSIGNAL, 1);
