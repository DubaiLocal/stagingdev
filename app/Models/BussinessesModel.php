...
class BussinessesModel extends Model
{
	...
	public function single_bussiness($slug)
	{
		...
		$builder->join('business_category', 'business_category.business_id = business.id');
		$builder->join('sub_category', 'sub_category.id = business_category.subcategory_id');
	...
	public function single_bussiness_meta($slug)
	{
		$db      = \Config\Database::connect();
		$data_array = array(
			"business.slug" => $slug,
		...
		$builder->select('district.name as location');
		$builder->select('sub_category.name as sub_cat_name');
		$builder->select('category.name as cat_name');
		$builder->join('district', 'district.id = business.district_id', 'left');
		$builder->join('business_category', 'business_category.business_id = business.id');
		$builder->join('sub_category', 'sub_category.id = business_category.subcategory_id');
		$builder->join('category', 'sub_category.cat_id = category.id');
		$db = $builder->where($data_array);
